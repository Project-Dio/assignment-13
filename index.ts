...
const todoContainer = document.getElementById("tasks-todo") as HTMLDivElement;
...
function createTaskCard(task: Task): HTMLDivElement {
  ...
  card.innerHTML = `
    ...
    <div class="flex items-center justify-between gap-2 mt-4">
      ...
      <span
        class="text-xs font-medium px-2.5 py-1 rounded-full capitalize ${getPriorityClasses(
          task.priority,
        )}"
      >
        ${task.priority}
...
function addCardEventListeners(card: HTMLDivElement, task: Task): void {
  const menuButton = card.querySelector(".task-menu-btn") as HTMLButtonElement;

  const menu = card.querySelector(".task-menu") as HTMLDivElement;

  const editButton = card.querySelector(".edit-task-btn") as HTMLButtonElement;

  const deleteButton = card.querySelector(
    ".delete-task-btn",
  ) as HTMLButtonElement;

  const statusSelect = card.querySelector(
    ".status-select",
  ) as HTMLSelectElement;
  ...
  statusSelect.addEventListener("change", () => {
    updateTaskStatus(task.id, statusSelect.value as TaskStatus);
  });
...
function renderColumn(container: HTMLDivElement, columnTasks: Task[]): void {
  container.innerHTML = "";
...
function setupDragAndDrop(): void {
  columns.forEach((column) => {
    ...
    const container = column.querySelector(`[id^="tasks-"]`) as HTMLDivElement;
